...
- **如果这次要改的地方和上次修复完全相同，或只是微调等待时间** → 说明没找到真正根因
- **修复无实质变化 → 立即停止，进入询问用户流程**，不要第 2、3 次重试

**第三步：有效修复才执行**
1. 生成**有实质变化**的修复代码（改定位器/加关弹窗/换等待策略）→ 保存 → 重新执行
2. **记录修复**：输出「🔧 修复记录：根因 → 改动 → 结果」
3. 重新执行后**若仍失败且修复无新变化** → 停止询问，最多 2 次实质修复

**关键点**：
- **诊断优先**：先搞清楚失败原因，不盲目改代码
- **修复增益**：修复无实质变化就不重试，避免"反复执行却修不好"
- **区分可修/不可修**：token 失效、外部网站不可用是环境问题，改脚本没用，直接问用户
- **有限重试**：最多 2 次**实质**修复，不是固定 3 次盲目重试
- **保存修复**：每次修复后立即保存到数据库

### 4.5 修复失败后询问用户（重要）

**触发条件**（任一即触发）：
- 修复 2 次后仍失败，且**修复无实质变化**
- 诊断出根因是**token 失效 / 外部网站不可用 / 环境问题**（改脚本无效）
- 测试卡死无法修复

**执行**：
1. **立即停止继续执行**（不再重试、不再静默）
2. **输出明确的询问消息**给用户，包含：
   - 失败根因（诊断结果）
   - 已尝试的修复（如果有）
   - 需要用户决策的选项：
     ```
     ⚠️ 测试脚本 test-x.spec.ts 执行失败
     根因：[token失效 / 弹窗遮挡 / 定位器失效 / 外部网站不可用 / 其他]
     已尝试：[列出修复]

     需要我：
     A) 提供新的 SESSION_TOKEN 后重试（若 token 失效）
...
**关键点**：
- **绝不盲目反复执行**：修复无变化或根因不可修，就停下询问
- **给出选项**：让用户快速决策
- **清晰沟通**：把根因、已尝试、选项都展示给用户
...
2. **先诊断根因**（同第 4️⃣ 第一步）：用 browser_snapshot 看页面卡在哪（加载中？弹窗？网络？）
3. **判断可修复性**：
   - 外部网站无响应/加载卡死 → 环境问题，**直接询问用户**（提供新 URL/跳过），不反复重跑
   - token 失效导致卡在登录 → 询问用户要新 token
   - 脚本等待过久 → 才有实质修复（缩短等待）
4. **修复有实质变化才重跑**（遵循修复增益原则），最多 2 次
5. **仍卡死 → 询问用户**：按第 4.5 流程给出选项（跳过/继续/提供环境信息）
6. **继续后续用例**：不因单个用例卡死阻塞整个测试任务

**关键点**：
- **超时即返回**：绝不无限等待，超时后必须给出结论
- **诊断优先**：先看卡在哪，判断是环境问题还是脚本问题
- **不盲目重跑**：环境问题（网站不可用/token失效）改脚本没用，直接问用户
- **有限重试**：最多 2 次**实质**修复
