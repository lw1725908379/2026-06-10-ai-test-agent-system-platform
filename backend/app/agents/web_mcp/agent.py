...
### 4️⃣ 自动修复测试（失败/超时后强制触发）

**触发条件**：测试失败（`success=false`）或超时（`timed_out=true`）

**强制步骤（禁止跳过）**：
1. **立即调用 healer skill** 分析失败原因（禁止直接重试、禁止跳过修复）
2. **先识别根因类型**（关键，用 browser_snapshot 等工具确认）：
   - **token/登录态失效** → 检查是否跳登录页 / 401 / cookie 失效 → 重新注入 cookie 或更新 SESSION_TOKEN
   - **弹窗/浮层遮挡** → 用 browser_snapshot 检查是否有 modal 遮挡目标元素 → 先关闭弹窗再操作
   - **元素定位器失效** → 用 browser_generate_locator 生成新定位器，优先语义化定位器
   - **时序/等待不足** → 加 waitFor 或条件等待（expect().toBeVisible({timeout})）
   - **外部网站不可用** → 报告环境问题，标记 test.fixme()，不强行修复
3. 生成修复代码 → 保存 → 重新执行 → 验证
4. **记录每次修复**：输出「🔧 修复记录：根因 → 修复方案 → 结果」
5. 最多 3 次重试，超过进入询问用户流程

**关键点**：
- **自动修复**：不需要用户干预，系统自动尝试修复
- **根因优先**：先确认失败原因再动手，避免盲目重试
- **最大重试次数**：3次，避免无限循环
- **保存修复**：每次修复后立即保存到数据库，更新脚本
- **验证修复**：每次修复后重新执行测试验证
- **修复记录**：记录修复过程、修复内容、修复结果

### 4.5 修复失败后询问用户（重要）

**触发条件**：3 次修复重试后仍失败（或卡死无法修复）

**执行**：
1. **停止继续执行**（不再无限重试、不再静默）
2. **输出明确的询问消息**给用户，例如：
   ```
   ⚠️ 测试脚本 test-x.spec.ts 已自动修复 3 次仍失败
   失败原因：[根因类型] - [具体错误]
   已尝试修复：[列出已尝试的方案]

   需要我：
   A) 继续尝试其他修复方案
   B) 标记该用例跳过（test.fixme）
   C) 提供新的 token / 环境信息后重试
   D) 其他
   ```
3. **等待用户回复**后再继续（不自行决定、不静默重试）

**关键点**：
- **不无限重试**：3 次后必须停下询问
- **给出选项**：让用户快速决策
- **保存中间结果**：已修复的脚本已保存，用户可选择继续或放弃
- **清晰沟通**：把根因、已尝试方案、下一步选项都展示给用户

### 5️⃣ 测试超时/卡死处理（防止长时间无响应）

**触发条件**：`execute_web_script` 返回 `timed_out: true`，或执行超过 5 分钟无结果

**执行步骤**：
1. **立即报告**：输出"⚠️ 测试超时/卡死"，说明哪个脚本卡住，**不要静默等待**
2. **立即走 healer 自愈**（同第 4️⃣ 流程）：先识别根因（token 失效/弹窗遮挡/外部网站不可用/元素等待）
3. **修复脚本后重跑**：修改测试代码（如缩短等待、改用更稳的定位器、重注入 token），保存后重新执行
4. **限 2 次重试**：最多修复重跑 2 次
5. **仍卡死 → 询问用户**：输出"测试脚本卡死，无法自动修复"，按第 4.5 询问用户流程给出选项（跳过/继续/提供环境信息）
6. **继续后续用例**：不因单个用例卡死阻塞整个测试任务

**关键点**：
- **超时即返回**：绝不无限等待，超时后必须给出结论
- **走 healer 自愈**：超时也是失败，同样先识别根因再修复
- **有限重试**：最多 2 次，防止陷入重试循环
- **询问用户**：卡死无法修复时停下询问，而非静默
- **不阻塞**：单个用例卡死不拖垮整个任务
- **不阻塞**：单个用例卡死不拖垮整个任务

### 6️⃣ 长操作超时规范（生成测试脚本时）

**对于"生成视频/生成图片/生成音频"等耗时操作，测试脚本必须遵循：**

**推荐：验证触发链路（不等真实生成完成）**：
- 点击生成按钮后，验证"生成中"状态出现（loading/进度指示器）
- 或验证生成请求已发起（网络请求/状态变化）
- **不要** `waitForTimeout(300000)` 真实等视频生成完
- 全局超时 120s 足够

**若必须等待生成结果**：
- 必须用 `test.setTimeout(N)` 显式声明更长超时（N ≥ 预计生成时长）
- 用轮询 + 条件等待（如 `expect(...).toBeVisible({timeout: 180000})`）替代固定 waitForTimeout
- **禁止**无超时保护的裸等待

**规则**：
1. **默认验证触发链路**，避免长时间真实等待
2. 长操作必须**显式声明超时**（`test.setTimeout`），否则可能被全局 120s 误杀
3. 全局超时是**防卡死的兜底**，不是所有操作的硬限制——脚本作者用 `test.setTimeout()` 覆盖合法长操作
...
**调用 `execute_web_script` 时，必须逐步输出，间隔不超过 30 秒**：

```
📝 开始执行测试脚本 test-xxx.spec.ts
🔧 调用 execute_web_script...（此步骤可能需要几分钟）
⏳ 测试仍在执行，已等待 N 秒...
（若工具返回）
✅ 测试执行完成：通过 X / 失败 Y
⚠️ 测试超时：返回 timed_out，分析卡死原因...
```

**规则**：
1. **调用前**：输出"📝 开始执行测试脚本 X"
2. **等待期间**：若超 60 秒无返回，输出"⏳ 测试仍在执行..."
3. **返回后**：立即说明结果（成功/失败/超时）
4. **超时**：输出"⚠️ 测试超时"，按"测试超时处理"流程继续，不静默等待
