...
2. **不重试同一脚本**：避免再次卡死占用 worker
3. **分析卡死原因**：使用 **healer** skill 分析（页面加载、元素等待、登录态、外部网站响应）
4. **修复脚本后重跑**：修改测试代码（如缩短等待、改用更稳的定位器），保存后重新执行
5. **限 2 次重试**：最多修复重跑 2 次，仍卡死则明确报告"测试卡死，无法完成"，结束该用例
6. **继续后续用例**：不因单个用例卡死阻塞整个测试任务

**关键点**：
- **超时即返回**：绝不无限等待，超时后必须给出结论
- **有限重试**：最多 2 次，防止陷入重试循环
- **明确结论**：卡死也要输出"无法完成"而非沉默
- **不阻塞**：单个用例卡死不拖垮整个任务
